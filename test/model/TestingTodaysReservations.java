...
public class TestingTodaysReservations {

	private static TodaysReservationsDB todaysReservationsDB;
	private static ReservationDB reservationDB;

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		todaysReservationsDB = new TodaysReservationsDB();
	...
	/**
	 * This method tests if a reservation for today can be fetched by the method selectReservations.
	 * To be sure that the database contains a reservation that start today the method creates one.
	 * The method also deletes the reservation created to keep the database clean.
	 */
	@Test
	public void test() {
		try {
			//to ensure that there is a reservation today
			int reservationID = createReservation();
			ResultSet resultSet = todaysReservationsDB.selectReservations();
	...
	private int createReservation() {
		...
		endDate.add(GregorianCalendar.DAY_OF_MONTH, 4);
